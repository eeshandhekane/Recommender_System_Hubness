...
for i in range(dataSize):
    temp = dataList[10000054/2+i].split("::")
    userInd = long(temp[0])
    movieInd = int(temp[1])
    ratings[i] = (temp[2])
...
print matrix[5000027-1]
print ratings[5000027-1]
np.save("Ratings3part2.npy", ratings)
np.save("Matrix3part2.npy", matrix)
np.save("Matrix3.npy" ,np.append(np.load("Matrix3part1.npy"), matrix, axis=0))
np.save("Ratings3.npy" ,np.append(np.load("Ratings3part1.npy"), ratings))
